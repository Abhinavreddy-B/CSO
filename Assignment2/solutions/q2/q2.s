...
.continue1:         # if n>=|diff| then it is possible to create such number
    ...
    jne .continue2  # if n==1 , continue from line 36
    
    cmpq $0,%r8     # compare |diff| with 0
    jne .continue2  # if n==1 and |diff|!=0 continue from line 36
...
.continue2:         # if n!=1|| (n==1 and |diff|!=0)
    cmpq $1,%r12    # compare n with 1
    jne .continue3  # if n!=1 continue from line 48
                    # otherwise if n==1 and |diff|==1 ( since n==1 and |diff|==0 is already taken care on line 25)
                    # n==1 and |diff|>=2 are alredy taken care on line 10
...
.continue3:         # if n!=1 and |diff|>=2
    ...
    movq $13,%r9    # move the value of 13 to register %r9 as say variable k
    
    movq %r14,%rax  # move x to %rax for division
    cqto            # Convert quadword in ​%rax​ to octoword in​ ​%rdx:%rax
    idivq %r9       # perform x/k and x%k
